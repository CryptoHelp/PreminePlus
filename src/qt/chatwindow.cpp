    ...
    : QWidget(parent), ui(new Ui::ChatWindowClass)
{
    ...
    //Style tooltips
    setStyleSheet("QToolTip { color: #ffffff; background-color: #2a82da; border: 1px solid white;padding: 6px; }");

	connect(ui->buttonConnect, SIGNAL(clicked()), this, SLOT(connecte()));
//    connect(ui->buttonConnect, SIGNAL(clicked()), ui->tab, SLOT(tabChanged(int)) ); //too soon
...
void ChatWindow::tabChanged(int index)
{
	if(index!=0 && joining == false)
        currentTab()->updateUsersList(ui->tab->tabText(index)); //Add this to initial operation
}
